...
const schema = z.object({
  ...
  probabilidad:  z.enum(['baja', 'media', 'alta', 'muy_alta']),
...
const probabilidadColor: Record<ProbabilidadCierre, 'danger' | 'warning' | 'info' | 'success'> = {
  baja: 'danger',
  media: 'warning',
  alta: 'info',
  muy_alta: 'success',
...
const OportunidadModal = ({ open, onClose, oportunidad }: OportunidadModalProps) => {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors }, reset } = useForm<FormData>({
    ...
    defaultValues: oportunidad
      ? {
          ...
          fechaEstimada: oportunidad.fechaEstimada.split('T')[0],
          descripcion: oportunidad.descripcion ?? '',
          notas: oportunidad.notas ?? '',
        }
      : { etapa: 'contacto', probabilidad: 'media' },
  })
  ...
  const probOpts = Object.keys(probabilidadColor).map(v => ({ value: v, label: v.charAt(0).toUpperCase() + v.slice(1) }))
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Select id="opp-prob" label="Probabilidad *" options={probOpts} value={watch('probabilidad')} onChange={v => setValue('probabilidad', v as ProbabilidadCierre)} />
        </div>
    ...
    <Card hoverable className="p-3 cursor-move hover:shadow-lg">
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-surface-500 mt-0.5">{opp.clienteNombre}</p>
        </div>
      ...
      <div className="space-y-2 pt-2 border-t border-white/[0.06]">
        <div className="flex items-center justify-between">
          ...
          <Badge variant={probabilidadColor[opp.probabilidad]} dot size="sm">{opp.probabilidad}</Badge>
        </div>
        <div className="text-xs text-surface-500">
          {format(new Date(opp.fechaEstimada), 'd MMM', { locale: es })}
        </div>
      </div>
    </Card>
  </motion.div>
...
export default function PipelinePage() {
  ...
  const valorTotalPorEtapa = etapas.reduce((acc, etapa) => ({
    ...
    [etapa]: oportunidades
      ...
      .reduce((sum, o) => sum + (o.valor * (o.probabilidad === 'baja' ? 0.2 : o.probabilidad === 'media' ? 0.5 : o.probabilidad === 'alta' ? 0.75 : 1)), 0),
